...
async function main() {
    ...
    for (const projectDir of projectDirs) {
        ...
        try {
            ...
            console.log(
                `[${projectName}] xcell generate completed successfully!`,
            );
            ...
            if (fs.existsSync(unityGeneratedDir)) {
                ...
                console.log(
                    `[${projectName}] Unity: ${files.length} files generated`,
                );
            } else {
                console.warn(
                    `[${projectName}] Unity generated files directory not found`,
                );
            }
            ...
            if (fs.existsSync(cocosGeneratedDir)) {
                ...
                console.log(
                    `[${projectName}] Cocos: ${files.length} files generated`,
                );
            } else {
                console.warn(
                    `[${projectName}] Cocos generated files directory not found`,
                );
            }
        } catch (error) {
            console.error(
                `[${projectName}] xcell generate failed:`,
                error.message,
            );
            continue;
        }
