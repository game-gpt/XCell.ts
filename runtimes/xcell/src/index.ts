...
export class XCell {
    ...
    private static async _initWithRetry(opts: XCellInitOptions): Promise<void> {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                if (attempt > 0) {
                    console.log(
                        `Retrying WASM load (attempt ${attempt}/${maxRetries})...`,
                    );
                ...
                console.error(
                    `WASM load attempt ${attempt + 1} failed:`,
                    lastError.message,
                );
    ...
    private static _setStatus(
        status: XCellLoadStatus,
        opts: XCellInitOptions,
    ): void {
        this._status = status;
